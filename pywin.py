...
class Frontend:
    ...
    def _handle_event_stream(self, event_type, _timestamp, *_args):
        ...
        if event_type == Events.BLINK.value:
                print('Blink!')
                win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN, screenPosX, screenPosY, 0, 0)
                win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP, screenPosX, screenPosY, 0, 0)
...
class TrackingWindow(QtWidgets.QWidget):
    ...
    def _handle_gaze_in_screen_stream(self, _timestamp, xpos, ypos):
        ...
        screenPosX = self._xcoord
        screenPosY = self._ycoord
        win32api.SetCursorPos((int(screenPosX), int(screenPosY - 30)))
